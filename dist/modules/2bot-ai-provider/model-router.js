...
exports.validateModelAvailable = validateModelAvailable;
exports.getSmartRoutingDecision = getSmartRoutingDecision;
exports.estimateSavings = estimateSavings;
const logger_1 = require("@/lib/logger");
const provider_config_1 = require("./provider-config");
const log = logger_1.logger.child({ module: "model-router" });
// ===========================================
...
function classifyQueryComplexity(messages) {
    ...
    let score = 0;
    const factors = []; // For debug logging
    ...
    log.debug({ score, factors, complexity, contentLength: content.length }, `Query classified as ${complexity.toUpperCase()}`);
    return complexity;
}
/**
 * Get model by tier from available models
 */
function getModelByTier(capability, preferredTier, provider) {
    let models = (0, provider_config_1.getAvailableModels)(capability);
 ...
 */
function getRecommendedModel(requestedModel, messages, allowDowngrade = true) {
    // First, check if requested model is available
    if (!(0, provider_config_1.isModelAvailable)(requestedModel)) {
        // Requested model not available - find alternative
        const cheapest = (0, provider_config_1.getCheapestModel)("text-generation");
        if (!cheapest) {
        ...
        log.warn({ requestedModel, fallback: cheapest.id }, "Requested model not available, using fallback");
        return cheapest.id;
    }
    ...
    const requestedModelInfo = (0, provider_config_1.getModelIfAvailable)(requestedModel);
    ...
    if (recommendedTier <= requestedTier) {
        log.info({
            ...
            complexity,
            savings: requestedModel !== recommendedModel.id
                ? "Using cheaper model!"
                : "Same model",
 ...
 */
function validateModelAvailable(modelId) {
    const model = (0, provider_config_1.getModelIfAvailable)(modelId);
    if (!model) {
        const available = (0, provider_config_1.getAvailableModels)("text-generation");
 ...
 */
function getSmartRoutingDecision(requestedModel, messages, allowDowngrade = true) {
    // First, check if requested model is available
    if (!(0, provider_config_1.isModelAvailable)(requestedModel)) {
        const cheapest = (0, provider_config_1.getCheapestModel)("text-generation");
        if (!cheapest) {
 ...
 */
function estimateSavings(originalModel, routedModel, inputTokens, outputTokens) {
    const original = (0, provider_config_1.getModelIfAvailable)(originalModel);
    const routed = (0, provider_config_1.getModelIfAvailable)(routedModel);
    if (!original || !routed) {
