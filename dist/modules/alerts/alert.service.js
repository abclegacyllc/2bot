...
const log = logger_1.logger.child({ module: 'alerts' });
...
class AlertServiceImpl {
    ...
    async checkAlerts(organizationId) {
        ...
        for (const resource of resources) {
            ...
            if (percentage >= 100) {
                ...
                const alert = await this.createAlert(organizationId, {
                    type: alert_types_1.AlertType.QUOTA_CRITICAL,
                    severity: alert_types_1.AlertSeverity.CRITICAL,
                ...
                const alert = await this.createAlert(organizationId, {
                    type: alert_types_1.AlertType.QUOTA_WARNING,
    ...
    async createAlert(organizationId, alertData) {
        ...
        await prisma_1.prisma.alertHistory.create({
            data: {
                id: alert.id,
                organizationId,
                type: alert.type,
        ...
        // Set cooldown
        await redis_1.redis.setex(cooldownKey, ALERT_COOLDOWN_SECONDS, '1');
        // Send notifications
        await this.sendAlertNotifications(organizationId, alert);
        log.info({ organizationId, alertId: alert.id, type: alert.type, severity: alert.severity }, 'Alert created and sent');
        return alert;
    }
    /**
     * Send alert via configured channels
     */
    async sendAlertNotifications(organizationId, alert) {
        const config = await this.getAlertConfig(organizationId);
     ...
     * Send email alert
     */
    async sendEmailAlert(email, orgName, alert) {
        try {
            const severityEmoji = {
                [alert_types_1.AlertSeverity.INFO]: 'ℹ️',
                [alert_types_1.AlertSeverity.WARNING]: '⚠️',
     ...
     * Send Telegram alert
     */
    async sendTelegramAlert(chatId, orgName, alert) {
        try {
...
_Organization: ${orgName}_
     ...
     * Send webhook alert
     */
    async sendWebhookAlert(webhookUrl, organizationId, alert) {
        try {
     ...
     */
    async getAlertConfig(organizationId) {
        ...
        // Check database
        const dbConfig = await prisma_1.prisma.alertConfig.findUnique({
        ...
        if (dbConfig) {
            const config = {
                ...
                channels: dbConfig.channels,
                enabled: dbConfig.enabled,
                createdAt: dbConfig.createdAt,
                updatedAt: dbConfig.updatedAt,
            };
            // Cache for 5 minutes
            await redis_1.redis.setex(cacheKey, 300, JSON.stringify(config));
            return config;
        }
        // Return defaults
        return {
    ...
    async getAlertHistory(organizationId, options) {
        ...
        const alerts = await prisma_1.prisma.alertHistory.findMany({
            where,
    ...
    async acknowledgeAlert(alertId, userId) {
        await prisma_1.prisma.alertHistory.update({
            where: { id: alertId },
            data: {
                acknowledged: true,
     ...
     */
    async getAlertStats(organizationId) {
        const alerts = await prisma_1.prisma.alertHistory.findMany({
            where: { organizationId },
     ...
     */
    async checkAllOrganizationAlerts() {
...
exports.alertService = new AlertServiceImpl();
