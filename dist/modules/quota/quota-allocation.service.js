...
exports.QuotaAllocationService = void 0;
...
const log = logger_1.logger.child({ module: 'quota-allocation' });
// ===========================================
// Permission Helpers
// ===========================================
...
function requireAdminRole(role, action) {
    if (!role || !ADMIN_ROLES.includes(role)) {
        throw new errors_1.ForbiddenError(`Admin access required to ${action}`);
    }
}
function requireManagerRole(role, action) {
    if (!role || !MANAGER_ROLES.includes(role)) {
        throw new errors_1.ForbiddenError(`Manager access required to ${action}`);
    }
}
// ===========================================
// Quota Allocation Service
...
class QuotaAllocationServiceImpl {
     ...
     */
    async setDeptAllocation(ctx, departmentId, alloc) {
        ...
        const validation = await this.validateDepartmentAllocation(dept.organizationId, alloc, departmentId // exclude this dept from calculation
        );
        if (!validation.valid) {
            throw new errors_1.AppError(`Allocation exceeds organization pool: ${validation.errors.map(e => e.message).join(', ')}`, 'ALLOCATION_EXCEEDED', 400);
        }
        // Upsert the allocation
        const deptAlloc = await prisma_1.prisma.deptAllocation.upsert({
            where: { departmentId },
            create: {
                departmentId,
                ...
                allocMode: alloc.allocMode ?? client_1.AllocationMode.SOFT_CAP,
                setById: ctx.userId,
            },
            update: {
                maxGateways: alloc.maxGateways ?? null,
        ...
        // Audit log
        await (0, audit_1.audit)(ctx, {
            action: 'quota.dept_allocation.set',
            resource: 'department',
            resourceId: departmentId,
            metadata: { allocation: alloc },
        });
        log.info({ departmentId, userId: ctx.userId, allocation: alloc }, 'Department allocation set');
        return {
            id: deptAlloc.id,
     ...
     */
    async getDeptAllocation(departmentId) {
     ...
     */
    async getDeptAllocations(organizationId) {
        const deptAllocs = await prisma_1.prisma.deptAllocation.findMany({
            where: {
     ...
     */
    async removeDeptAllocation(ctx, departmentId) {
        const dept = await prisma_1.prisma.department.findUnique({
            where: { id: departmentId },
        ...
        await prisma_1.prisma.deptAllocation.delete({
            where: { departmentId },
        });
        await (0, audit_1.audit)(ctx, {
     ...
     */
    async setMemberAllocation(ctx, userId, departmentId, alloc) {
        ...
        if (role === 'DEPT_MANAGER') {
            const deptMember = await prisma_1.prisma.departmentMember.findUnique({
                where: {
                    userId_departmentId: { userId: ctx.userId, departmentId },
                },
            });
            if (!deptMember) {
                throw new errors_1.ForbiddenError('Can only manage allocations in your department');
            }
        }
        // Verify user is in this department
        const targetDeptMember = await prisma_1.prisma.departmentMember.findUnique({
            where: {
        ...
        const validation = await this.validateMemberAllocation(departmentId, alloc, userId // exclude this user from calculation
        );
        if (!validation.valid) {
            throw new errors_1.AppError(`Allocation exceeds department pool: ${validation.errors.map(e => e.message).join(', ')}`, 'ALLOCATION_EXCEEDED', 400);
        }
        // Upsert the allocation
        const memberAlloc = await prisma_1.prisma.memberAllocation.upsert({
            where: {
                userId_departmentId: { userId, departmentId },
            },
            create: {
        ...
        log.info({ userId, departmentId, setBy: ctx.userId, allocation: alloc }, 'Member allocation set');
        return {
            id: memberAlloc.id,
     ...
     */
    async getMemberAllocation(userId, departmentId) {
     ...
     */
    async getMemberAllocations(departmentId) {
        const memberAllocs = await prisma_1.prisma.memberAllocation.findMany({
            where: { departmentId },
            include: {
                user: { select: { name: true, email: true } },
                department: { select: { name: true } },
     ...
     */
    async removeMemberAllocation(ctx, userId, departmentId) {
        const dept = await prisma_1.prisma.department.findUnique({
            where: { id: departmentId },
            select: { organizationId: true },
        });
        ...
        await prisma_1.prisma.memberAllocation.delete({
            where: {
                userId_departmentId: { userId, departmentId },
            },
        });
        ...
        log.info({ userId, departmentId, removedBy: ctx.userId }, 'Member allocation removed');
    }
    // =========================================
    // Validation
    // =========================================
     ...
     */
    async validateDepartmentAllocation(organizationId, allocation, excludeDeptId) {
        const errors = [];
        ...
        const currentAllocs = await prisma_1.prisma.deptAllocation.findMany({
            where: {
                department: { organizationId },
        ...
        for (const check of checks) {
            ...
            const newTotal = currentlyAllocated + requested;
            if (newTotal > limit) {
                errors.push({
                    field: check.field,
     ...
     */
    async validateMemberAllocation(departmentId, allocation, excludeUserId) {
        const errors = [];
        // Get department allocation
        const deptAlloc = await prisma_1.prisma.deptAllocation.findUnique({
            where: { departmentId },
        ...
        const currentAllocs = await prisma_1.prisma.memberAllocation.findMany({
            where: {
                departmentId,
     ...
     */
    async getUnallocatedOrgResources(organizationId) {
        const org = await prisma_1.prisma.organization.findUnique({
            where: { id: organizationId },
            select: { plan: true },
        });
        ...
        const planLimits = org_plans_1.ORG_PLAN_LIMITS[org.plan];
        if (!planLimits) {
            throw new errors_1.AppError('Unknown organization plan', 'INVALID_PLAN', 400);
        }
        const deptAllocs = await prisma_1.prisma.deptAllocation.findMany({
     ...
     */
    async getUnallocatedDeptResources(departmentId) {
        const deptAlloc = await prisma_1.prisma.deptAllocation.findUnique({
            where: { departmentId },
        });
     ...
     */
    async getOrgAllocationSummary(organizationId) {
        const org = await prisma_1.prisma.organization.findUnique({
            where: { id: organizationId },
            select: { plan: true },
        });
     ...
     */
    async getDeptAllocationSummary(departmentId) {
...
// Export singleton
exports.QuotaAllocationService = new QuotaAllocationServiceImpl();
//# sourceMappingURL=quota-allocation.service.js.map