...
exports.trackWriteOperations = trackWriteOperations;
const logger_1 = require("@/lib/logger");
const quota_1 = require("@/modules/quota");
const log = logger_1.logger.child({ module: 'usage-middleware' });
/**
...
function trackApiUsage(req, res, next) {
    ...
    quota_1.usageTracker.trackApiCall(trackingCtx).catch((err) => {
        log.error({ err, userId: ctx.userId, path: req.path }, 'Failed to track API call');
    });
    // Track errors on response finish
    res.on('finish', () => {
        if (res.statusCode >= 500) {
            quota_1.usageTracker.trackError(trackingCtx, `HTTP_${res.statusCode}`).catch((err) => {
 ...
 */
function trackWriteOperations(req, res, next) {
