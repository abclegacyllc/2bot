...
/**
 * Studio › Versions tab — list `ProjectVersion` rows with Activate / Rollback.
 */
...
export default function VersionsTabPage() {
  ...
  const handleActivate = async (versionId: string) => {
    ...
    try {
      const res = await activateProjectVersion(
        projectId,
        versionId,
        token ?? undefined,
      );
    ...
    <div className="container mx-auto max-w-5xl space-y-6 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Each apply of a BuildSpec creates a STAGING version. Activate to
            promote; roll back to revert to a previous snapshot.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {!loading && versions.length === 0 && !error ? (
        <div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">
          No versions yet. Use Cursor or apply a BuildSpec to create one.
        </div>
      ) : null}
            ...
            >
              <div className="min-w-0 flex-1 space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {v.source ? (
                    <span className="text-xs text-muted-foreground">via {v.source}</span>
                  ) : null}
                </div>
