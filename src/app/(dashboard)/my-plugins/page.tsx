...
function MyPluginsContent() {
  ...
  const handleToggle = async (id: string, enabled: boolean) => {
    ...
    try {
      // Use org endpoint if in org context, otherwise use personal endpoint
      const toggleUrl = isOrgContext && orgId
        ? apiUrl(`/orgs/${orgId}/plugins/${id}/toggle`)
        : apiUrl(`/user/plugins/${id}/toggle`);
  ...
  const handleSaveConfig = async (config: Record<string, unknown>) => {
    ...
    try {
      // Use org endpoint if in org context, otherwise use personal endpoint
      const configUrl = isOrgContext && orgId
        ? apiUrl(`/orgs/${orgId}/plugins/${configuringPlugin.id}/config`)
        : apiUrl(`/user/plugins/${configuringPlugin.id}/config`);
  ...
  const handleUninstall = async (id: string) => {
    ...
    try {
      // Use org endpoint if in org context, otherwise use personal endpoint
      const deleteUrl = isOrgContext && orgId
        ? apiUrl(`/orgs/${orgId}/plugins/${id}`)
        : apiUrl(`/user/plugins/${id}`);
