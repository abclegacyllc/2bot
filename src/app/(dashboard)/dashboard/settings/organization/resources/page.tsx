 ...
 *
 * @module app/dashboard/settings/organization/resources/page
 */
...
function OwnerDashboardContent() {
  ...
  const isOwner = context.type === "organization" && context.orgRole === "ORG_OWNER";

  // Redirect if not owner
  useEffect(() => {
    if (context.type !== "organization") {
      router.push("/dashboard");
    } else if (context.orgRole !== "ORG_OWNER") {
      router.push("/dashboard");
    }
  }, [context, router]);

  const fetchData = useCallback(async () => {
    if (!token || context.type !== "organization") return;
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Button asChild variant="outline" size="sm">
                      <Link
                        href={`/dashboard/settings/organization/departments/${dept.id}/quotas`}
                      >
                        Edit Quota
                      </Link>
                    </Button>
