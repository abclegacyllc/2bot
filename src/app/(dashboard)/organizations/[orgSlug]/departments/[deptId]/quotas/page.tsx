...
function DeptQuotaContent() {
  ...
  const isOwnerOrAdmin = orgRole === "ORG_OWNER" || orgRole === "ORG_ADMIN";
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const deptRes = await fetch(apiUrl(`/departments/${departmentId}`), {
      ...
      const quotaRes = await fetch(apiUrl(`/departments/${departmentId}/quotas`), {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      // Fetch org limits
      const orgLimitsRes = await fetch(apiUrl("/quota/limits"), {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const onSubmit = async (data: QuotaFormInput) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl(`/departments/${departmentId}/quotas`), {
        method: "PUT",
        headers: {
