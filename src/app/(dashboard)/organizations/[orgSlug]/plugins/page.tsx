...
/**
 * Organization Plugins Page
 *
 * Displays the plugin catalog with install/uninstall actions for organization.
...
function PluginsContent() {
  ...
  useEffect(() => {
    ...
    load();
  }, [isFound, orgId, fetchPlugins, fetchInstalledPlugins]);
  ...
  const handleInstall = async (slug: string) => {
    ...
    try {
      const response = await fetch(apiUrl(`/orgs/${orgId}/plugins/install`), {
        method: "POST",
        headers: {
  ...
  const handleUninstall = async (slug: string) => {
    ...
    if (!installed || !orgId) return;

    setInstallingSlug(slug);
    try {
      const response = await fetch(apiUrl(`/orgs/${orgId}/plugins/${installed.id}`), {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-foreground">{orgName ? `${orgName} - ` : ""}Available Plugins</h1>
