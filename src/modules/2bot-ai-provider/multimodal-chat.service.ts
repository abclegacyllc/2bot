...
import { twoBotAIProvider } from "./2bot-ai.provider";
...
import type { TextGenerationMessage, TwoBotAIModel } from "./types";

const log = logger.child({ module: "multimodal-chat" });
...
const conversations = new Map<string, MultimodalConversation>();
 ...
 */
export function detectCapabilities(input: MultimodalChatInput): DetectedIntent {
  const detected: AICapability[] = [];
  let primaryCapability: AICapability = "text-generation";
  let confidence = 0.5;
  ...
  if (input.text) {
    ...
    if (
      /\b(generate|create|make|draw|design|paint)\s+(an?\s+)?(image|picture|photo|illustration)/i.test(text) ||
    ...
    // TTS patterns
    if (
      /\b(read\s+(this\s+)?(aloud|out\s*loud)|say\s+this|speak|text[\s-]*to[\s-]*speech|tts)\b/i.test(text)
    ) {
...
function createImageBlock(
  ...
  url: string,
  options?: Partial<ImageContentBlock>
): ImageContentBlock {
  return {
    type: "image",
    id: nanoid(),
...
export async function multimodalChat(
  ...
  if (input.images) {
    for (const img of input.images) {
      userMessage.content.push(
        createImageBlock("uploaded", img.url || `data:${img.mimeType};base64,${img.base64}`)
      );
    }
  }
    ...
    switch (intent.primaryCapability) {
      ...
      case "image-understanding": {
        ...
        const response = await twoBotAIProvider.textGeneration({
          messages,
          model: (conversation.defaultModels["text-generation"] || "gpt-4o-mini") as TwoBotAIModel,
          userId,
          organizationId,
      ...
      case "image-generation": {
        ...
        for (const img of response.images) {
          assistantMessage.content.push(
            createImageBlock("generated", img.url, {
              prompt,
        ...
        if (response.images[0]?.revisedPrompt) {
          assistantMessage.content.push(
            createTextBlock(`I generated an image based on your request. The prompt I used was: "${response.images[0].revisedPrompt}"`)
          );
        }
      ...
      case "speech-synthesis": {
        ...
        const response = await twoBotAIProvider.speechSynthesis({
          text: textToSpeak,
          model: "tts-1",
          voice: conversation.settings.ttsDefaults?.voice as "alloy" || "alloy",
        ...
        assistantMessage.content.push(
          createTextBlock(`I've converted your text to speech. You can play it above.`)
        );
      ...
      case "speech-recognition": {
        ...
        assistantMessage.content.push(
          createTextBlock(`Here's the transcription of your audio:\n\n"${response.text}"`)
        );
      ...
      case "video-generation": {
        ...
        assistantMessage.content.push(
          createTextBlock("Video generation is coming soon! This capability will be available when providers like OpenAI Sora become publicly available.")
        );
  ...
  let newBalance = 0;
  try {
    const { twoBotAICreditService } = await import("@/modules/credits");
    if (organizationId) {
...
function buildChatMessages(
  ...
  currentInput: MultimodalChatInput
 ...
 */
export async function* multimodalChatStream(
  request: MultimodalChatRequest
): AsyncGenerator<MultimodalChatStreamChunk> {
  ...
  for (const block of response.message.content) {
    if (block.type === "text") {
      ...
      yield {
        type: block.type as "image" | "audio",
        content: block,
      };
    }
  }
...
export const multimodalChatService = {
  chat: multimodalChat,
