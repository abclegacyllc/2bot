...
import { createAnalyticsStorage, type AnalyticsConfig, type AnalyticsEvent } from "./index";

const analyticsLogger = logger.child({ module: "analytics-plugin" });
...
export class AnalyticsPlugin extends BasePlugin {
  ...
  readonly tags: string[] = ["analytics", "statistics", "telegram", "tracking"];
  ...
  async onEvent(
    ...
    const config = context.config as Partial<AnalyticsConfig>;
    const storage = createAnalyticsStorage(context.userPluginId, config);
    ...
    try {
      ...
      if (this.isTelegramMessage(event)) {
        ...
        return this.success(
          { tracked: true, eventType: "message.received" },
      ...
      if (this.isTelegramCallback(event)) {
        ...
        return this.success(
          { tracked: true, eventType: "callback.received" },
          { durationMs: Date.now() - startTime }
        );
      }
      ...
      if (this.isWorkflowStep(event)) {
        const input = event.data.input as { action?: string; days?: number; limit?: number } | undefined;
      ...
      analyticsLogger.warn(
        { userPluginId: context.userPluginId, eventType: event.type },
        "Unsupported event type for analytics plugin"
      );

      return this.success(
        { tracked: false, reason: "unsupported_event_type" },
        { durationMs: Date.now() - startTime }
      );
    } catch (error) {
      analyticsLogger.error(
        { err: error, userPluginId: context.userPluginId },
  ...
  async onInstall(context: PluginContext): Promise<void> {
    analyticsLogger.info(
      { userPluginId: context.userPluginId, userId: context.userId },
  ...
  async onUninstall(context: PluginContext): Promise<void> {
    analyticsLogger.info(
      { userPluginId: context.userPluginId, userId: context.userId },
      "Analytics plugin uninstalling - cleaning up data"
    );
   ...
   */
  private mapTelegramMessageToEvent(
    event: Extract<PluginEvent, { type: "telegram.message" }>
  ): AnalyticsEvent {
    return {
   ...
   */
  private mapTelegramCallbackToEvent(
    event: Extract<PluginEvent, { type: "telegram.callback" }>
  ): AnalyticsEvent {
    return {
