 ...
 *
 * @module modules/cursor/buildspec/orchestrator.service
 */
...
import type { ExternalApiCredentials } from "@/modules/project-resource/project-resource.types";
import { ensureDefaultProject } from "@/modules/project/project.service";
import { createServiceContext } from "@/shared/types/context";

import type { BuildSpecGeneratedPluginInstall } from "./buildspec.schema";
...
const builderLogger = logger.child({ module: "cursor-buildspec" });
...
interface RollbackHandles {
  ...
  /** Plugin catalog rows created by Wave 2 codegen (`source: "generated"`). */
  pluginCatalogIds: string[];
  gatewayIds: string[];
...
async function resolvePluginInstalls(
  ...
  for (const inst of spec.plugins) {
    if (inst.source === "generated") {
      await installGeneratedPlugin(owner, inst, projectId, refMap, rollback);
      continue;
    }
...
/**
 * Wave 2 codegen: materialise a generated plugin from a B5 template and
 * install it as a USER-authored Plugin + UserPlugin pair.
 *
 * Flow:
 *   1. Render via `renderTemplate(...)` (pure, deterministic).
 *   2. Defence-in-depth: validate every emitted .js / .mjs file against the
 *      same `validatePluginCode` denylist used at install time, so a
 *      malicious template input cannot inject `child_process` or `eval`.
 *   3. Refuse if the slug already exists in the catalog (Plugin.slug is
 *      globally unique). Wave 2 callers must rename via `pluginSlug`.
 *   4. Create the catalog Plugin row (`authorType: "USER"`, isPublic=false).
 *   5. Create the UserPlugin install pointing at the new catalog row.
 *   6. Best-effort write the rendered files into the user's running
 *      workspace container. If the workspace is offline the install still
 *      succeeds — `pluginDeployService.ensureFileExists` will recover the
 *      files from `Plugin.codeBundle` / `Plugin.manifest` on first run.
 *
 * Both the new Plugin row and UserPlugin row are pushed onto the rollback
 * chain so a smoke-test failure cleanly removes them.
 */
async function installGeneratedPlugin(
  owner: BuildSpecOwner,
  inst: BuildSpecGeneratedPluginInstall,
  projectId: string,
  refMap: BuildSpecApplyResult["refMap"],
  rollback: RollbackHandles,
): Promise<void> {
  ...
  const rendered = renderTemplate({
    ...
    slug: inst.pluginSlug,
    name: inst.name,
    description: inst.description ?? null,
    inputs: inst.templateInputs,
  });

  // 2. Defence-in-depth code validation on every emitted JS file.
  for (const file of rendered.files) {
    if (/\.(js|mjs|cjs)$/.test(file.path)) {
      validatePluginCode(file.content, rendered.slug);
    }
  }

  // 3. Slug collision check — Plugin.slug is globally unique.
  const collision = await prisma.plugin.findUnique({
    where: { slug: rendered.slug },
    select: { id: true },
  });
  if (collision) {
    throw new Error(
      `Generated plugin slug "${rendered.slug}" already exists in the catalog. ` +
        "Choose a different `pluginSlug` in the BuildSpec.",
    );
  }

  // 4. Create the catalog Plugin row. `codeBundle` holds the entry-file
  //    contents so single-file recovery still works; `manifest` stores the
  //    full plugin.json for directory-layout recovery via the catalog.
  const entryFileContent =
    rendered.files.find((f) => f.path === rendered.entryFile)?.content ?? "";

  const catalogPlugin = await prisma.plugin.create({
    data: {
      slug: rendered.slug,
      name: rendered.manifest.name,
      description: rendered.manifest.description,
      version: rendered.manifest.version,
      requiredGateways: rendered.manifest.requiredGateways,
      configSchema: rendered.manifest.configSchema as object,
      icon: rendered.manifest.icon,
      category: rendered.manifest.category,
      tags: rendered.manifest.tags,
      isBuiltin: false,
      authorType: "USER",
      authorId: owner.userId,
      isPublic: false,
      manifest: rendered.manifest as unknown as object,
      codeBundle: entryFileContent,
      eventTypes: rendered.manifest.eventTypes,
      eventRole: rendered.manifest.eventRole,
      inputSchema: (rendered.manifest.inputSchema ?? undefined) as object | undefined,
      outputSchema: (rendered.manifest.outputSchema ?? undefined) as object | undefined,
    },
  });
  rollback.pluginCatalogIds.push(catalogPlugin.id);

  // 5. Resolve the optional gateway binding and compute the workspace
  //    entry path. Generated templates always use directory layout.
  const gatewayId = inst.gatewayRef
    ? refMap.gateways[inst.gatewayRef] ?? null
    : null;
  ...
  if (gatewayId) {
    ...
    platform = gw ? gatewayTypeToPlatform(gw.type) : undefined;
  }

  const installEntryFile = getPluginEntryPath(gatewayId, catalogPlugin.slug, {
    platform,
    isDirectory: true,
    entry: rendered.entryFile,
  });

  // 6. Create the UserPlugin install row.
  const userPlugin = await prisma.userPlugin.create({
    data: {
      ...
      gatewayId,
      config: (inst.config ?? {}) as object,
      entryFile: installEntryFile,
      isEnabled: true,
    },
  });
  refMap.plugins[inst.ref] = userPlugin.id;
  rollback.userPluginIds.push(userPlugin.id);

  // 7. Best-effort workspace deploy. If the workspace is offline this
  //    silently returns false; the file recovery path in plugin-deploy
  //    will materialise the files from the catalog on first event.
  const filesForDeploy: Record<string, string> = {};
  let manifestJson: string | null = null;
  for (const f of rendered.files) {
    if (f.path === "plugin.json") {
      manifestJson = f.content;
    } else {
      filesForDeploy[f.path] = f.content;
    }
  }
  if (manifestJson === null) {
    manifestJson = JSON.stringify(rendered.manifest, null, 2);
  }
  ...
  try {
    ...
    builderLogger.warn(
      ...
      "Generated plugin workspace deploy failed; install row created — files will be recovered from catalog on demand",
    );
  }
}
...
async function rollbackAll(rb: RollbackHandles): Promise<void> {
  ...
  // Drop generated catalog Plugin rows AFTER their UserPlugin installs are
  // gone (FK constraint). Marketplace/builtin plugins are NEVER pushed onto
  // pluginCatalogIds so this only affects Wave 2 codegen rollbacks.
  if (rb.pluginCatalogIds.length > 0) {
    await prisma.plugin.deleteMany({ where: { id: { in: rb.pluginCatalogIds } } });
  }
