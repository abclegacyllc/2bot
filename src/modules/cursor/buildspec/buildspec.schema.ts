 ...
 *
 * @module modules/cursor/buildspec/buildspec.schema
 */
...
// UserPlugin install
//
// Wave 1 only supported `source: "marketplace"` (install an existing
// catalog plugin by slug). Wave 2 introduces `source: "generated"` —
// the AI authors a new plugin's code from a code template (see
// `code-templates/` library + Wave 2 codegen pipeline).
//
// Old specs that omit `source` are auto-upgraded to `marketplace` by
// the preprocess shim below so existing tests / external callers keep
// validating.
// ─────────────────────────────────────────────────────────────────────

const MarketplacePluginInstall = z.object({
  source: z.literal("marketplace"),
  ref: SpecLocalId,
  /**
   * Marketplace plugin slug (e.g. `"echo-bot"`). Must already exist in the
   * `Plugin` catalog table — this arm does NOT create plugin code.
   */
  pluginSlug: z.string().min(1).max(120),
  /** Optional gateway binding by spec-local ref. */
  gatewayRef: SpecLocalId.optional(),
  config: z.record(z.string(), z.unknown()).optional(),
});

const GeneratedPluginInstall = z.object({
  source: z.literal("generated"),
  ref: SpecLocalId,
  /**
   * Slug for the NEW plugin record that will be created in the catalog.
   * Must be unique within the owner's catalog at apply time.
   */
  pluginSlug: z
    .string()
    .min(1)
    ...
    .regex(/^[a-z0-9](?:[a-z0-9-]{0,118}[a-z0-9])?$/,
      "pluginSlug must be lowercase, dashed, 1-120 chars",
    ),
  /** Human-readable plugin name written into the new catalog row. */
  name: z.string().min(1).max(120),
  description: z.string().max(2000).nullable().optional(),
  /**
   * Identifier of the code template to materialise, e.g.
   * `"telegram-echo-typescript"`. Resolved against the template
   * library at apply time (Phase B5).
   */
  template: z.string().min(1).max(120),
  /**
   * Free-form values consumed by the template engine (replaces
   * placeholders, drives optional sections). Validated per-template
   * by the codegen pipeline — kept open here so the schema layer
   * stays template-agnostic.
   */
  templateInputs: z.record(z.string(), z.unknown()).optional(),
  /** Optional gateway binding by spec-local ref. */
  gatewayRef: SpecLocalId.optional(),
  /** Initial UserPlugin install config (validated against generated configSchema at runtime). */
  config: z.record(z.string(), z.unknown()).optional(),
});

export const PluginInstallSpec = z.preprocess(
  (val) => {
    // Backward-compat: when a spec was authored before the discriminated
    // union landed (no `source` field), default to "marketplace".
    if (
      val &&
      typeof val === "object" &&
      !Array.isArray(val) &&
      ...
      return { source: "marketplace", ...(val as Record<string, unknown>) };
    }
    return val;
  },
  z.discriminatedUnion("source", [
    MarketplacePluginInstall,
    GeneratedPluginInstall,
  ]),
);
...
// Smoke tests
//
// Wave 1: only `kind: "preflight"` (static graph + plugin-binding check,
// no execution).
// Wave 2 adds two execution-based kinds:
//   • `manual-run`     — executes the workflow once with empty triggerData,
//                        as if a user clicked "Run now".
//   • `sample-payload` — executes the workflow once with a caller-supplied
//                        `payload` (used as the trigger event body, e.g. a
//                        Telegram update or webhook body).
//
// Both Wave 2 kinds run with `{ dryRun: true, allowDraft: true }` so:
//   • no real outbound gateway side-effects fire (Telegram sendMessage etc.),
//   • the smoke test can target the freshly-created DRAFT workflow row.
//
// Specs that omit `kind` keep validating: the discriminator preprocesses
// missing `kind` to `"preflight"` (Wave 1 backward compat).
// ─────────────────────────────────────────────────────────────────────

const SmokeTestPreflight = z.object({
  workflowRef: SpecLocalId,
  kind: z.literal("preflight"),
});

const SmokeTestManualRun = z.object({
  workflowRef: SpecLocalId,
  kind: z.literal("manual-run"),
  /** Optional inline trigger data; defaults to {} if omitted. */
  payload: z.record(z.string(), z.unknown()).optional(),
});

const SmokeTestSamplePayload = z.object({
  workflowRef: SpecLocalId,
  kind: z.literal("sample-payload"),
  /** Required payload — the synthetic trigger event body. */
  payload: z.record(z.string(), z.unknown()),
});

export const SmokeTestSpec = z.preprocess(
  (val) => {
    if (val && typeof val === "object" && !Array.isArray(val)) {
      const obj = val as Record<string, unknown>;
      if (obj.kind === undefined) {
        return { ...obj, kind: "preflight" };
      }
    }
    return val;
  },
...
export type BuildSpecMarketplacePluginInstall = z.infer<
  typeof MarketplacePluginInstall
>;
export type BuildSpecGeneratedPluginInstall = z.infer<
  typeof GeneratedPluginInstall
>;
